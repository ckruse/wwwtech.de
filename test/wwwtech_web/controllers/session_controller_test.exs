defmodule WwwtechWeb.SessionControllerTest do
  ...
  test "redirects to index when login data is valid", %{conn: conn, author: author} do
    conn = post(conn, session_path(conn, :create), session: %{"email" => author.email, "password" => "abcd"})
    assert redirected_to(conn) == "/"
  end

  test "renders form when login data is wrong", %{conn: conn, author: author} do
    conn = post(conn, session_path(conn, :create), session: %{"email" => author.email, "password" => "111"})
    assert html_response(conn, 200) =~ "<h2>Login</h2>"

    conn = post(conn, session_path(conn, :create), session: %{"email" => "foo bar", "password" => "abcd"})
    assert html_response(conn, 200) =~ "<h2>Login</h2>"
  end
